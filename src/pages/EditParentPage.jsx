...
import { editSecondaryParent, getAllSecondaryParents, getChildLinkRequests } from "../api/childApi";
...
export default function EditParentPage() {
  ...
  useEffect(() => {
    const loadParentData = async () => {
      try {
        setIsLoading(true);
        const response = await getAllSecondaryParents();
        const parents = response?.data?.data?.secondaryParents || [];
        const currentParent = parents.find(parent => parent.id == parentId);
        
        if (currentParent) {
          setParentData(currentParent);
          setUserFilters([currentParent.receiverId]);
          setSelectedFilters([currentParent.relationshipTypeId]);
          setSelectedResidence(currentParent.sameResidence);
          setCustomRelationship(currentParent.customRelationship || "");
        } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!userFilters || userFilters.length === 0) {
        toast.error("Please select a parent to edit");
        return;
      }
      ...
      // Validate data structure for edit API
      console.log("📋 Edit parent data validation:");
      ...
      console.log("📤 Sending edit data to API:", formattedData);
      
      // Make the API call
             const response = await editSecondaryParent(parentId, formattedData.receiverId, formattedData);
       console.log("✅ Edit parent API Success:", response.data);
       toast.success(response?.data?.data?.data?.message || "Secondary parent updated successfully!");
       navigate("/manage-relation");
      ...
      if (error?.response?.status === 401) {
        ...
        toast.error("Parent not found. Please check the parent ID.");
      } else if (error?.response?.data?.message) {
        toast.error(error.response.data.message);
      } else if (error?.message) {
        toast.error(`Request failed: ${error.message}`);
      } else {
        toast.error("Failed to update secondary parent. Please try again.");
      }
    } finally {
        ...
        <div className="row">
          <div className="col-lg-12">
            <div className="text-center mt-5">
              ...
              <p className="mt-3">Loading parent data...</p>
            </div>
          </div>
        </div>
      </div>
